/**
 * Lädt Appwrite-Variablen für CLI-Skripte: zuerst process.env,
 * dann Repo-Root `.env.local` / `.env.migration`, optional `functions/.env`.
...
/**
 * Sucht `.env.local` / `.env` / `.env.migration` beginnend bei `startDir` nach oben
...
function collectEnvPathsWalkingUp(startDir) {
  ...
  for (let depth = 0; depth < 14; depth++) {
    for (const name of [".env.local", ".env", ".env.migration"]) {
