...
const env = { ...parseEnv(readFileSync(serverEnvPath, "utf8")), ...process.env };
...
const FUNCTIONS = [
  "scriptony-ai","scriptony-assistant","scriptony-audio","scriptony-auth",
  "scriptony-beats","scriptony-characters","scriptony-clips","scriptony-gym",
  "scriptony-image","scriptony-logs","scriptony-mcp-appwrite","scriptony-project-nodes",
  "scriptony-projects","scriptony-shots","scriptony-stage","scriptony-stage2d",
  "scriptony-stats","scriptony-style","scriptony-style-guide","scriptony-superadmin",
  "scriptony-video","scriptony-worldbuilding",
];
...
async function syncVariable(functions, fnId, desired, value) {
  ...
  if (!existing) {
    await functions.createVariable({ functionId: fnId, key: desired.key, value, secret: desired.secret });
    return "created";
  }

  // If secret flag matches, just update
  if (existing.secret === desired.secret) {
    await functions.updateVariable({ functionId: fnId, variableId: existing.$id, key: desired.key, value, secret: desired.secret });
    return "updated";
  }

  // Secret flag mismatch — delete and recreate
  await functions.deleteVariable({ functionId: fnId, variableId: existing.$id });
  await functions.createVariable({ functionId: fnId, key: desired.key, value, secret: desired.secret });
  return "recreated";
}
