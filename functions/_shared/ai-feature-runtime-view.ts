...
import { getModelsForProvider, getModelInfo } from "./ai-service/config";
import { PROVIDER_DISPLAY_NAMES, isOllamaFamilyProvider } from "./ai-service/providers";
...
function inferOllamaMode(runtime: ResolvedFeatureRuntime): "local" | "cloud" | undefined {
...
function providerDisplayName(runtime: ResolvedFeatureRuntime, ollamaMode?: "local" | "cloud"): string {
  if (runtime.config.provider === "ollama" && ollamaMode === "cloud") return "Ollama (Cloud)";
  if (runtime.config.provider === "ollama" && ollamaMode === "local") return "Ollama (lokal)";
  return PROVIDER_DISPLAY_NAMES[runtime.config.provider] || runtime.config.provider;
}

function credentialError(runtime: ResolvedFeatureRuntime, ollamaMode?: "local" | "cloud"): string | undefined {
  ...
  if (runtime.config.provider === "ollama_local" || (runtime.config.provider === "ollama" && ollamaMode === "local")) {
    return "Ollama (lokal): Bitte Basis-URL in den KI-Integrationen setzen.";
  }
  if (runtime.config.provider === "ollama_cloud" || (runtime.config.provider === "ollama" && ollamaMode === "cloud")) {
    return "Ollama Cloud: Bitte API-Key in den KI-Integrationen hinterlegen.";
  }
  return `Bitte API-Key für ${providerDisplayName(runtime, ollamaMode)} in den KI-Integrationen hinterlegen.`;
}

export function describeResolvedFeatureRuntime(runtime: ResolvedFeatureRuntime): ResolvedFeatureRuntimeStatus {
...
function toRuntimeModels(rows: ReturnType<typeof getModelsForProvider>): RuntimeModelInfo[] {
  return rows.map((row) => ({
    id: row.id,
    name: row.name,
    context_window: typeof row.contextWindow === "number" && row.contextWindow > 0 ? row.contextWindow : 8192,
  }));
}

function findModelByIdLoose(list: RuntimeModelInfo[], id: string): RuntimeModelInfo | undefined {
...
function defaultContextWindow(provider: string, list: RuntimeModelInfo[], modelId: string): number {
  const registryInfo = getModelInfo(modelId);
  if (typeof registryInfo?.contextWindow === "number" && registryInfo.contextWindow > 0) {
    return registryInfo.contextWindow;
  }
  if (list.length > 0 && list[0].context_window > 0) return list[0].context_window;
  if (provider === "deepseek") return 64000;
  return 128000;
}

function modelDisplayName(provider: string, modelId: string): string {
  const registryInfo = getModelsForProvider(provider).find((row) => row.id === modelId) || getModelInfo(modelId);
...
async function fetchModelsForRuntime(
  ...
  const registryModels = toRuntimeModels(getModelsForProvider(runtime.config.provider));
  if (!status.hasCredentials) {
    return { models: registryModels, source: "registry" };
  }

  const providerForFetch =
    runtime.config.provider === "ollama_local" || runtime.config.provider === "ollama_cloud"
      ? "ollama"
  ...
  const remote = await fetchUnifiedModels({
    ...
    ...(status.ollamaMode === "local" ? { ollamaBaseUrl: runtime.baseUrl } : {}),
  });
...
export async function getFeatureRuntimeView(
  ...
  models = mergeSelectedModelIfMissing(runtime.config.provider, runtime.config.model, models);
  const selectedModel = runtime.config.model.trim()
    ? findModelByIdLoose(models, runtime.config.model) || {
        id: runtime.config.model.trim(),
        name: modelDisplayName(runtime.config.provider, runtime.config.model.trim()),
        context_window: defaultContextWindow(runtime.config.provider, models, runtime.config.model.trim()),
      }
    : null;
