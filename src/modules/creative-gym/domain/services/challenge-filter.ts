...
export function filterChallenges(
  ...
  if (filters.maxDurationMin != null) {
    list = list.filter((c) => c.recommendedDurationMin <= filters.maxDurationMin!);
  }
  if (filters.minDurationMin != null) {
    list = list.filter((c) => c.recommendedDurationMin >= filters.minDurationMin!);
  }
  ...
  if (filters.sessionMode) {
    list = list.filter(
      (c) => c.defaultSessionMode === filters.sessionMode || c.tags.includes(filters.sessionMode!)
    );
  }
