...
const FILM_THREE_ACT: NonNullable<NarrativeInitBody['predefinedNodes']>['level_1'] = [
...
const FILM_FOUR_ACT: NonNullable<NarrativeInitBody['predefinedNodes']>['level_1'] = [
  { number: 1, title: 'Akt I - Setup', description: 'Einführung: Figuren, Welt, Störung' },
  { number: 2, title: 'Akt IIa - Konfrontation (1)', description: 'Erste Hälfte der Konfrontation' },
  { number: 3, title: 'Akt IIb - Konfrontation (2)', description: 'Zweite Hälfte der Konfrontation' },
  { number: 4, title: 'Akt III - Auflösung', description: 'Finale und Auflösung' },
];

const BOOK_THREE_PART: NonNullable<NarrativeInitBody['predefinedNodes']>['level_1'] = [
...
function theaterFiveActLevel1(): NonNullable<NarrativeInitBody['predefinedNodes']>['level_1'] {
...
function heroesJourneyLevel1(): NonNullable<NarrativeInitBody['predefinedNodes']>['level_1'] {
...
function saveTheCatLevel1(): NonNullable<NarrativeInitBody['predefinedNodes']>['level_1'] {
...
export function narrativeStructureInitUiHint(
  ...
  if (tid === 'film-heroes-journey') return { kind: 'ready', shortLabel: 'Heldenreise anlegen' };
  if (tid === 'film-save-the-cat') return { kind: 'ready', shortLabel: 'Save-the-Cat-Raster anlegen' };
  if (tid === 'book-novel') return { kind: 'ready', shortLabel: '3-Teile-Struktur anlegen' };
  if (tid === 'film-4-act') return { kind: 'ready', shortLabel: '4-Akt-Struktur anlegen' };
  if (tid === 'film-5-act') return { kind: 'ready', shortLabel: '5-Akt-Struktur anlegen' };
