...
function mergeMetadataPct(
  ...
  return (acts || []).map((act: any) => {
    ...
    return {
      ...act,
      metadata: { ...(act.metadata || {}), pct_from: o.pct_from, pct_to: o.pct_to },
    };
  });
}
...
function mergeSeqScene(
  ...
  return {
    sequences: (sequences || []).map((seq: any) => {
      ...
      return {
        ...seq,
        metadata: { ...(seq.metadata || {}), pct_from: o.pct_from, pct_to: o.pct_to },
      };
    }),
...
export function buildEffectiveFilmTimelineData(
  timelineData: { acts?: any[]; sequences?: any[]; scenes?: any[]; shots?: any[] } | null,
  timings: FilmManualTimings
): { acts: any[]; sequences: any[]; scenes: any[]; shots: any[] } | null {
  if (!timelineData) return null;
  const withAct = mergeMetadataPct(timelineData.acts || [], timings.manualActTimings);
...
export function computeFilmShotSpans(
  ...
  for (const [sceneId, sceneShots] of shotsByScene.entries()) {
    ...
    const storedDurations = orderedShots.map((sh) => getShotDurationSec(sh, timings.manualShotDurations));
